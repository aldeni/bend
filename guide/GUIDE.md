...
`TCP.listen_shared` sets `SO_REUSEPORT`, so N processes that all call it share
one port; on Linux the kernel spreads accepted connections over them, while on
macOS the bind is permitted but the first listener takes every connection.
