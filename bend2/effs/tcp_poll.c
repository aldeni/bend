...
static Term tcp_poll_step(Env e, IoWork* w, Term (*more)(Env, IoWork*),
  ...
  return tcp_poll_end(e, w, w->code ? io_fail(e, w->code, NULL) : io_done(e,
    io_box(e, CID(Some), pack(e, w->data, w->size))));
}

static void tcp_poll_start(Term* f, IoWork* w) {
...
static Term tcp_poll_more(Env e, IoWork* w) {
  return tcp_poll_step(e, w, tcp_poll_more, io_str);
}
...
static Term tcp_poll_bytes_more(Env e, IoWork* w) {
  return tcp_poll_step(e, w, tcp_poll_bytes_more, io_list);
}
